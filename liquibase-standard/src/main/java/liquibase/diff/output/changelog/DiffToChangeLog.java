...
public class DiffToChangeLog {
    ...
    private String queryForDependenciesPostgreSql(List<String> schemas) {
        ...
        return "WITH RECURSIVE preference AS (\n" +
                ...
                "           '^pg_(conversion|language|ts_(dict|template))'::pg_catalog.text AS class_exclusion,\n" +
                ...
                "        SELECT oid, oid::regclass::pg_catalog.text AS object_name  \n" +
                "        FROM pg_class \n" +
                "    )\n" +
                "    SELECT DISTINCT \n" +
                "        pg_catalog.substring(pg_identify_object(classid, objid, 0)::pg_catalog.text, E'(\\w+?)\\.') AS referenced_schema_name, \n" +
                "        CASE classid\n" +
                "            WHEN 'pg_constraint'::regclass THEN (SELECT contype::pg_catalog.text FROM pg_constraint WHERE oid = objid)\n" +
                "            ELSE objid::pg_catalog.text\n" +
                "        END AS CONTYPE,\n" +
                "        CASE classid\n" +
                "            WHEN 'pg_attrdef'::regclass THEN \n" +
                "                (SELECT attname FROM pg_attrdef d \n" +
                "                 JOIN pg_attribute c ON (c.attrelid, c.attnum) = (d.adrelid, d.adnum) \n" +
                "                 WHERE d.oid = objid)\n" +
                "            WHEN 'pg_cast'::regclass THEN \n" +
                "                (SELECT concat(castsource::regtype::pg_catalog.text, ' AS ', casttarget::regtype::pg_catalog.text,' WITH ', castfunc::regprocedure::pg_catalog.text) \n" +
                "                 FROM pg_cast WHERE oid = objid)\n" +
                ...
                "            WHEN 'pg_proc'::regclass THEN objid::regprocedure::pg_catalog.text\n" +
                "            WHEN 'pg_rewrite'::regclass THEN (SELECT ev_class::regclass::pg_catalog.text FROM pg_rewrite WHERE oid = objid)\n" +
                "            WHEN 'pg_trigger'::regclass THEN (SELECT tgname FROM pg_trigger WHERE oid = objid)\n" +
                "            WHEN 'pg_type'::regclass THEN objid::regtype::pg_catalog.text\n" +
                "            ELSE objid::pg_catalog.text\n" +
                "        END AS REFERENCED_NAME,\n" +
                "        pg_catalog.substring(pg_identify_object(refclassid, refobjid, 0)::pg_catalog.text, E'(\\w+?)\\.') AS referencing_schema_name, \n" +
                "        CASE refclassid\n" +
                ...
                "            WHEN 'pg_proc'::regclass THEN refobjid::regprocedure::pg_catalog.text\n" +
                "            WHEN 'pg_type'::regclass THEN refobjid::regtype::pg_catalog.text\n" +
                "            ELSE refobjid::pg_catalog.text\n" +
                ...
                "      AND classid::regclass::pg_catalog.text !~ preference.class_exclusion\n" +
                "      AND refclassid::regclass::pg_catalog.text !~ preference.class_exclusion\n" +
                "      AND COALESCE(pg_catalog.SUBSTRING(objid::regclass::pg_catalog.text, E'^(\\w+)\\.'), '') !~ preference.schema_exclusion\n" +
                "      AND COALESCE(pg_catalog.SUBSTRING(refobjid::regclass::pg_catalog.text, E'^(\\w+)\\.'), '') !~ preference.schema_exclusion\n" +
                "    GROUP BY classid, objid, refclassid, refobjid, deptype, rel.object_name, rrel.object_name\n" +
                ")\n" +
                "SELECT referenced_schema_name,\n" +
                "       (CASE\n" +
                "          WHEN position('.' in referenced_name) > 0 \n" +
                "          THEN substring(referenced_name from position('.' in referenced_name) + 1 for length(referenced_name))\n" +
                "          ELSE referenced_name\n" +
                "       END) AS referenced_name, \n" +
                "       referencing_schema_name,\n" +
                "       (CASE\n" +
                "          WHEN position('.' in referencing_name) > 0 \n" +
                "          THEN substring(referencing_name from position('.' in referencing_name) + 1 for length(referencing_name))\n" +
                "          ELSE referencing_name\n" +
                "       END) AS referencing_name\n" +
                "FROM dependency_pair\n" +
                "WHERE REFERENCED_NAME != REFERENCING_NAME\n" +
                "  AND (REFERENCED_NAME LIKE 'public.%' OR REFERENCED_NAME NOT LIKE '%.%')\n" +
                "  AND (CONTYPE::pg_catalog.text != 'p' AND CONTYPE::pg_catalog.text != 'f')\n" +
                "  AND referencing_schema_name IS NOT NULL \n" +
                "  AND referencing_name IS NOT NULL;";
    }
